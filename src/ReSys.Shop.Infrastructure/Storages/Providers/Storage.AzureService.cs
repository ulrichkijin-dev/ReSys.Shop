...
public sealed class AzureStorageService : IStorageService
{
    ...
    public AzureStorageService(IOptions<StorageOptions> options)
    {
        ...
        var (name, key) = ParseConnectionString(cs: _options.AzureConnectionString!);
        _storage = StorageFactory.Blobs.AzureBlobStorageWithSharedKey(accountName: name, key: key);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            Log.Error(exception: ex, messageTemplate: "Azure upload failed");
        ...
        CancellationToken ct)
    {
        ...
        foreach (var w in options.ThumbnailWidths)
        {
            ...
            var thumbPath = Path.ChangeExtension(path: basePath, extension: null) + $"_{w}" + Path.GetExtension(path: basePath);

            await _storage.WriteAsync(fullPath: thumbPath, dataStream: thumb, append: options.Overwrite, cancellationToken: ct);
    ...
    private string GetFileUrl(string path)
        => $"{_options.AzureCdnUrl?.TrimEnd(trimChar: '/')}/{path}";

    private string GetBlobPath(string url)
    {
        if (string.IsNullOrEmpty(value: _options.AzureCdnUrl))
            return url.Trim(trimChar: '/');

        return url
            .Replace(oldValue: _options.AzureCdnUrl, newValue: "", comparisonType: StringComparison.OrdinalIgnoreCase)
            .Trim(trimChar: '/');
    }

    private static (string, string) ParseConnectionString(string cs)
    {
        var parts = cs.Split(separator: ';', options: StringSplitOptions.RemoveEmptyEntries);
        var name = parts.First(predicate: p => p.StartsWith(value: "AccountName="))[12..];
        var key = parts.First(predicate: p => p.StartsWith(value: "AccountKey="))[11..];
        return (name, key);
    }
}
