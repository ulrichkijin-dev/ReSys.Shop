...
public static partial class SettingModule
{
    public sealed class Endpoints : ICarterModule
    {
        ...
        {
            var group = app.MapGroup(prefix: "api/admin/settings")
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: command,
                    cancellationToken: cancellationToken);
                var apiResponse =
                    result.ToApiResponseCreated(message: "Setting created successfully");
                return TypedResults.Ok(value: apiResponse);
            })
                .UseEndpointMeta(meta: Annotations.Create)
                .RequireAccessPermission(permission: FeaturePermission.Admin.Setting.SettingPermissions.Create);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: command,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponseCreated(message: "Setting updated successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.Update).RequireAccessPermission(permission: FeaturePermission.Admin.Setting.SettingPermissions.Update);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: command,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponseDeleted(message: "Setting deleted successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.Delete)
                .RequireAccessPermission(permission: FeaturePermission.Admin.Setting.SettingPermissions.Delete);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: query,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponse(message: "Settings retrieved successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.GetSelectList)
                .RequireAccessPermission(permission: FeaturePermission.Admin.Setting.SettingPermissions.List);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: query,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponse(message: "Setting retrieved successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.GetById)
            .RequireAccessPermission(permission: FeaturePermission.Admin.Setting.SettingPermissions.View);
