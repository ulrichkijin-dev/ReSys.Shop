...
class ThesisVisualizer:
    ...
    def plot_inference_performance(self, bench_df: pd.DataFrame):
        ...
        sns.scatterplot(data=bench_df, x='Avg Inference (ms)', y='mAP', 
                        hue='Model', s=200, style='Type')
        ...
        for i, row in bench_df.iterrows():
            plt.text(row['Avg Inference (ms)']+0.5, 
                     row['mAP'], 
                     row['Model'], 
                     fontsize=9)
    ...
    def generate_tsne(self, db: Session, model_name: str, n_samples: int = 500):
        ...
        sql = text(f"SELECT {emb_col}, product_id FROM product_images WHERE {emb_col} IS NOT NULL LIMIT {n_samples}")
        ...
        for row in data:
            ...
            if isinstance(vec, str):
                try:
                    ...
                    clean = vec.replace('np.str_', '').replace('"', '').replace("'", "").strip()
            ...
            if isinstance(vec, list) and len(vec) > 0:
                ...
                labels.append(SearchService.get_categorization(db, row[1])["article_type"])
        ...
        sns.scatterplot(data=df[df['Category'].isin(top_cats)], 
                        x='x', y='y', hue='Category', palette='tab10', alpha=0.6)
...
def benchmark_models(db: Session, sample_size=50):
    ...
    results = []
    images = db.query(ProductImage).filter(ProductImage.type == "Search").limit(sample_size).all()
    ...
    for m_name in settings.AVAILABLE_MODELS:
        ...
        # Warmup
        if images: embedder.extract_features(images[0].url)
